...
for k = 1:M(2)
    if M(2)-k+1 >= len   
        U2_tot{k} = zeros(M(2)-k+1,2^level); % output of analysis filters for 2nd order signal 
    else
...
for n = 1:ITER
    ...
    a2 = cat(1, reshape(un2_tap.*un(n),[1,1,M(2)]), a2(1:end-1,:,:));                 % Update tapped-delay line of u(n)
