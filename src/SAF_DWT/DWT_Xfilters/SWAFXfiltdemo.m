% SWAFXfiltdemo          Subband Wavelet-domain Adaptive Filter, witch crossfilter Demo
% 
...
title('Input Signal'); 
% axis([0 10 -1.5 1.5])
out_resp = SWAtest_crossfilt(delta, S); 
subplot(2,1,2)
stem(out_resp);
title('Output Signal-Estimated System vs True');
hold on; 
real_resp = filter(b, 1, delta);
stem(real_resp); legend('Estim', 'True'); legend('show')
% axis([0 2*M -1.5 1.5])
...
plot(input_sine);
title('Input Signal'); 
out_sine = SWAtest_crossfilt(input_sine, S); 
subplot(2,2,2)
plot(out_sine);
title('Output Signal - Estimated System vs True');
hold on; 
real_sys = filter(b,1,input_sine);
plot(real_sys); legend('Estim', 'True'); legend('show')
